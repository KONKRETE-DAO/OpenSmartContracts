...
contract Database is AccessControl {
    ...
    function addKycPermit(
        ...
        require(
            hasRole(SIGNER, ECDSA.recover(hash_, v, r, s)),
            "Invalid signer"
        );
 ...
 */
    ...
    function changeCountryCode(address customer, uint16 countryCode_)
        external
        onlyRole(DEV)
    {
    ...
    function changeCountryAuthorisation(uint16 toChange, bool canBuy_)
        external
        onlyRole(DEV)
    {
    ...
    function addKyc(address toKyc, uint16 countryCode_)
        external
        onlyRole(SIGNER)
    {
    ...
    function addKycs(address[] calldata toKyc, uint16[] calldata countryCodes)
        external
        onlyRole(DEV)
    {
    ...
    function addWhitelists(address[] calldata toWhitelist)
        external
        onlyRole(DEV)
    {
