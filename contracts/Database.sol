...
contract Database is AccessControlEnumerable {
    ...
    mapping(uint256 => bool) public cannotBuy;
        ...
        external
    {
        ...
        uint256 nonce_ = nonce[toKyc];

        bytes32 hash_ = ECDSA.toEthSignedMessageHash(
            keccak256(abi.encodePacked(toKyc, countryCode_, deadline, address(this), nonce_, block.chainid))
        );
    ...
    function addWhitelistPermit(address toWhitelist, uint64 deadline, bytes32 r, bytes32 s, uint8 v) external {
        ...
        bytes32 hash_ = ECDSA.toEthSignedMessageHash(
            keccak256(abi.encodePacked(toWhitelist, deadline, address(this), block.chainid))
        );
    ...
    function changeCountryCode(address customer, uint16 countryCode_) external onlyRole(DEV) {
    ...
    function changeCountryAuthorisation(uint16 toChange, bool canBuy_) external onlyRole(DEV) {
        cannotBuy[toChange] = !canBuy_;
    }

    function addKyc(address toKyc, uint16 countryCode_) external onlyRole(SIGNER) {
        require(countryCode[toKyc] == 0, "Already kyced");
        _addKyc(toKyc, countryCode_);
    }

    function addKycs(address[] calldata toKyc, uint16[] calldata countryCodes) external onlyRole(DEV) {
        uint256 kycLen = toKyc.length;
        require(kycLen > 0, "Empty array");
        require(kycLen == countryCodes.length, "Arrays length doesn't match");
        unchecked {
            for (uint256 i = 0; i < kycLen;) {
    ...
    function revokeKyc(address toUnKyc) external {
        require(hasRole(DEV, _msgSender()) || toUnKyc == _msgSender(), "Revoke Kyc: Not Authorized");
    ...
    function addWhitelists(address[] calldata toWhitelist) external onlyRole(DEV) {
        uint256 toWhitelistLen = toWhitelist.length;
        require(toWhitelistLen > 0, "Empty array");
        unchecked {
            for (uint256 i = 0; i < toWhitelistLen;) {
    ...
    function canBuy(address toCheck) public view returns (bool) {
        return !cannotBuy[countryCode[toCheck]] && (!whitelistActivated || isWhitelisted[toCheck]);
    }
