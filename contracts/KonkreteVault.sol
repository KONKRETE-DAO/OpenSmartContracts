 ...
 */
  function getStep() public view returns (SaleStep step) {
    ...
    if (block.timestamp < uint(depositsStart)) return SaleStep.PREAUCTION;
    if (block.timestamp < uint(depositsStop)) return SaleStep.SALE;
    if (collectedCapital < uint(softCap)) return SaleStep.SALE_FAILED;
  ...
  function _maxDeposit(address user) internal view returns (uint256) {
    ...
    uint maxDepositPerUser_ = uint(stepVar.maxDepositPerUser);

    uint256 paidByUser = paid[user];
    if (hardCap_ <= collectedK_ || maxDepositPerUser_ <= paidByUser) return 0;
