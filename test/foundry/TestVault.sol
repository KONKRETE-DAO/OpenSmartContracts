...
contract KonVaulSimpleTest is ContractTest {
    ...
    function testSimpleMintAndDepositSofCapx2() internal {
        ...
        require(
            vault.balanceOf(addr[1]) == softCap,
            "Wrong v Depositer Balance"
        );
        uint256 vaultBal = usdc.balanceOf(address(vault));
        require(
            vaultBal == vault.collectedCapital() &&
                vaultBal == softCap + softCap,
    ...
    function interestRollerCoaster(
        ...
        if ((!multiplierEqual0 && !collectedEqual0))
            interest = ((collectedCapital * int256(multiplierx100)) /
                int256(100));
        ...
        require(
            usdc.balanceOf(address(vault)) == bankBal,
            " Wrong vault balance"
        );
        ...
        uint256 expectedPrice = uint256(
            int256(stableMantissa) +
                int256(
                    ((int256(stableMantissa) * multiplierx100) / int256(100))
                )
        );
