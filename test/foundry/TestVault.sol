...
contract KonVaulSimpleTest is ContractTest {
     ...
     */
    function testMultiPleProcess(int16 multiplierx100_, uint64 amounts, uint8 softCapMultiplier) public {
    ...
    function testSimpleMintAndDepositSofCapx2() internal {
        ...
        require(vaultBal == vault.collectedCapital() && vaultBal == softCap + softCap, "WrongCollectedCapital");
    }

    function testInterestUpdate() internal {
        goToSaleWithOneWallet();
        vault.updateInterest(int256(softCap));
        require(vault.tokenPrice() / 2 == vault.originalPrice(), " WrongPriceRaise");
        vault.updateInterest(-int256(softCap));
    ...
    function interestRollerCoaster(int256 multiplierx100, uint256 amountsMintedAtTheFirstStep) internal {
        vm.prank(addr[BANK]);
        ...
        require(tokenPrice == vault.priceImpact(bankBal), "Price is not equal to priceImpact");
